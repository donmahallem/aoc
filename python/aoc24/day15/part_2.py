    ...
    for row in range(0, len(fieldData)):
        for col in range(0, len(fieldData[row])):
            if fieldData[row][col] == "#":
                ...
                field[row,
                      2 * col:2 * col + 2] = [CELL_BOX_LEFT, CELL_BOX_RIGHT]
...
def next_empty(field: np.typing.NDArray, cur_y: int, cur_x: int, dir_y: int,
               dir_x: int) -> tuple[int, int] | None:
    next_y, next_x = cur_y + dir_y, cur_x + dir_x
    if field[next_y, next_x] == CELL_WALL:
        return None
    elif dir_y != 0 and ((field[next_y, next_x] == CELL_BOX_LEFT
                          and field[cur_y, cur_x] == CELL_BOX_RIGHT) or
                         (field[next_y, next_x] == CELL_BOX_RIGHT
                          and field[cur_y, cur_x] == CELL_BOX_LEFT)):
        return None
    elif field[next_y, next_x] == CELL_EMPTY:
        if dir_y != 0:
            if (field[cur_y, cur_x] == CELL_BOX_LEFT
                    and field[next_y, next_x + 1] != CELL_EMPTY):
                return None
            elif (field[cur_y, cur_x] == CELL_BOX_RIGHT
                  and field[next_y, next_x - 1] != CELL_EMPTY):
                return None
...
def Part2(input: typing.TextIO) -> int:
    ...
    for move in movements:
        ...
        if field[next_y, next_x] == CELL_WALL:
            ...
            continue
        elif (field[next_y, next_x] == CELL_BOX_LEFT
            ...
            if move[1] != 0:
                ...
                if maybe_next_empty:
                    ...
                    if move[1] < 0:
                        field[next_y,
                              next_empty_x:next_x] = field[next_y,
                                                           next_empty_x +
                                                           1:next_x + 1]
                        field[next_y, next_x] = CELL_EMPTY
                    elif move[1] > 0:
                        field[next_y, next_x + 1:next_empty_x +
                              1] = field[next_y, next_x:next_empty_x]
                        field[next_y, next_x] = CELL_EMPTY
            elif move[0] != 0:
